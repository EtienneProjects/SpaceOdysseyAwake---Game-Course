...
class Asteroid(pygame.sprite.Sprite):
    def __init__(self, x, y, scale, health=20, is_fragment=False, ice=None, junk=None, fragment_count=1):
        ...
        # if ice type chance 
        # ice has a higher chance then junk even with the same (1,3), why? because code is executed top down so if both are true in same case ice gets created over junk
    ...
    def update(self, asteroid_group, player):
        ...
        #Check health
        if self.health <= 0:
            self.break_apart(asteroid_group, rocket_hit=False, space_strike=False)
            # add points to score 
            config.score += 10
    ...
    def break_apart(self, asteroid_group, rocket_hit=False, space_strike=False):
    ...
    def __init__(self, x=None, y=None, initial_size=160, rotation_speed=0.15, frame_rate_ms=100):
        ...
        if not self.frames:# if no image load one image as default
            try:
                ...
                # if still no image 0 first one in blackhole folder we create blannk alpha

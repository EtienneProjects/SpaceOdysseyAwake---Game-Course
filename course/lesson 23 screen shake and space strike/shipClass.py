...
class Ship(pygame.sprite.Sprite):
    def __init__(self, character_type, ship_x, ship_y, scale, velocity):
        ...
        self.health = 100 # ship and enemy health
        self.shield = 200
        ...
        self.rect.center = (ship_x, ship_y)
        
        
        #laser shooting attributes
        self.lasers = pygame.sprite.Group() #lasers created by this ship instance
    ...
    def check_alive(self, player):
        ...
        if self.health <= 0:
            if self.alive: # we transition from alive to dead\
                ...
                reward = config.enemy_rewards.get(self.character_type, {'score': 70,'shield': 30, 'health':0, 'space_score': 0})
                ...
                # add points to space points
                config.space_points_holder += reward.get('space_points', 0)
                if config.space_points_holder >= 200:
                    config.space_strike += 1
                    config.space_points_holder = 0
    ...
    def damage_flash(self):
        ...
        #Handlee flashing/shield animations
        if flashing:
    ...
    def ai_shoot_laserline(self, player, asteroid_group=None, laserline_group=None, blackholes=None):
        ...
        if not self.alive and laserline_group is not None: # if enemy has died and the is a laseline object so not of type None
            for line in list(laserline_group):
                if getattr(line, "ship", None) == self:
                    line.trigger(False)
    ...
    def shoot_plasma(self, target_group, asteroid_group, plasma_group):
        ...
        if now - self.last_plasma_time >= cooldown:
            ...
            offstets = [(-24,14), (30, 26)]
            
            for x_off, y_off in offstets:
...
class HealthBar():
    ...
    def draw(self, health, shield=False):
        ...
        if self.shield:
            pygame.draw.rect(game_window, BLACK, (self.healthBar_x -2, self.healthBar_y -2, 174, 9))
            pygame.draw.rect(game_window, WHITE, (self.healthBar_x, self.healthBar_y, 170, 6))
            pygame.draw.rect(game_window, CAYAN, (self.healthBar_x, self.healthBar_y, 170 * ratio, 6))
        else:
            pygame.draw.rect(game_window, BLACK, (self.healthBar_x -2, self.healthBar_y -2, 174, 9))
            pygame.draw.rect(game_window, RED, (self.healthBar_x, self.healthBar_y, 170, 6))
            pygame.draw.rect(game_window, GREEN, (self.healthBar_x, self.healthBar_y, 170 * ratio, 6))
...
# Create special enemy , use inheritance from base class ship
class Mothership(Ship):
    ...
    def update(self, player):
        ...
        if self.rect.left <= 0:
            self.rect.left = 0
            self.vx = abs(self.vx) # trow away sign to force x in positive or 0
        ...
        # preserve base class ship flash and shield effect 
        super().update(player)

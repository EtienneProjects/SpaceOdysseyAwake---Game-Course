...
def apply_damage(entity, dmg, damage_type="normal"):
    ...
    if hasattr(entity, "character_type") and hasattr(entity, "shield") :
        ...
        if shield > 0:
            ...
            if not getattr(config, "shake_active", False) and entity.character_type == 'player':           
                config.shake_active = True
                config.shake_start_time = pygame.time.get_ticks()
                config.shake_duration = 800 # 0.8 second
                config.shake_amplitude = 2
...
class Laser(pygame.sprite.Sprite):
    def __init__(self, shooter, player, enemy_group, asteroid_group, damage=10, velocity=12):
        ...
        if shooter.character_type.startswith("enemy"):
            self.image = pygame.transform.flip(self.image, False, True) # Enemy shoots from top downs so we need to flip in y
            self.direction = 1
        ...
        if not self.exploding:
            ...
            for asteroid in list(self.asteroid_group):
                if hasattr(asteroid, "rect") and detection_area.colliderect(asteroid.rect):
                    asteroid.health -= 100
                    asteroid.break_apart(self.asteroid_group, rocket_hit=True, space_strike=False)   
                    hit_something = True
    ...
    def animate_explosion(self):   
        ...
        if not self.damage_applied:                     
            ...
            for asteroid in list(self.asteroid_group):
                if hasattr(asteroid, "rect") and detection_area.colliderect(asteroid.rect):
                    asteroid.health -= 20
                    asteroid.break_apart(self.asteroid_group, rocket_hit=False, space_strike=False)
...
# Horizontal laser enemy long range strike (player must avoid it)
class HorizontalLaser(pygame.sprite.Sprite):
    def __init__(self, image, start_pos, direction, player, asteroid_group, enemy_group, 
                speed=25, damage_player=30, damage_asteroid=100, damage_enemy=200):
        super().__init__()
        ...
        self.damage_asteroid = damage_asteroid
    ...
    def update(self):
        ...
        if self.rect.colliderect(self.player.rect):
            apply_damage(self.player, self.damage_player)
            self.kill()
            return
        ...
        for enemy in self.enemy_group:
            if self.rect.colliderect(enemy.rect):
                apply_damage(enemy, self.damage_enemy)
                self.kill()
                return
        ...
        # kill long laser strike off screen
        if self.rect.right < 0 or self.rect.left > config.SCREEN_WIDTH:
            self.kill()   
        
        
        
# manage long range enemy laser attack
class HorizontalManager:
    def __init__(self, warning_img_path, laser_img_path, player, asteroid_group, enemy_group, interval_ms=10000, warning_scale=(300, 200)):
        self.original_warning_image = pygame.image.load(warning_img_path).convert_alpha()
        self.original_warning_image = pygame.transform.scale(self.original_warning_image, warning_scale)
        self.laser_image = pygame.image.load(laser_img_path).convert_alpha()
        ...
        self.original_warning_image = self.original_warning_image.copy()
        self.warning_active = False
        self.warning_start_time = 0
        self.warning_duration = 4500
        self.effect_active = False
        self.effect_start_time = 0
        self.effect_duration = 20000
        self.next_strike_time = pygame.time.get_ticks() + interval_ms
        self.interval_ms = interval_ms
        self.direction = 1
        
        self.lasers_to_spawn = []
        self.warning_rect = None
        self.warning_y_range = (0, 0) # min and max y of burst
    ...
    def update(self):
        ...
        # Trigger warning 
        if now >= self.next_strike_time and not self.warning_active and not self.effect_active:
            self.warning_active = True
            self.warning_start_time = now
            self.direction = 1 if random.random() < 0.5 else -1
            self.next_strike_time = now + self.interval_ms
            
            # flip warning sign for left moving lasers
            self.warning_image = self.original_warning_image.copy()
            if self.direction == -1:
                self.warning_image = pygame.transform.flip(self.warning_image, True, False)
            
            if self.direction == 1: # coming from left ---> to right
                warning_x = 0 + self.warning_image.get_width() // 2
            else:
                warning_x = screen_width - self.warning_image.get_width() // 2
                
            # set warning Y range anywhere within 100 px height 
            min_y = random.randint(50, screen_height - 150)
            max_y = min_y + 100
            self.warning_y_range = (min_y, max_y)
            # center warning image in the middle of range
            warning_center_y = (min_y + max_y) // 2
            self.warning_rect = self.warning_image.get_rect(center=(warning_x, warning_center_y))
            
        # start effect after warning
        if self.warning_active and now - self.warning_start_time >= self.warning_duration:
            self.warning_active = False
            self.effect_active = True
            self.effect_start_time = now
            
            
            # burst 15 - 22 lasers each time
            burst_count = random.randint(15, 22)
            self.lasers_to_spawn.clear()
            for i in range(burst_count):
                spawn_delay = i * 100
                y_pos = random.randint(self.warning_y_range[0], self.warning_y_range[1])
                self.lasers_to_spawn.append((now + spawn_delay, y_pos))
                
                
        # spawn lasers gradually off screen
        for spawn_time, y in self.lasers_to_spawn[:]:
            if now >= spawn_time:
                config.channel_13.set_volume(0.5)
                config.channel_13.play(config.long_fx)
                start_x = -25 if self.direction == 1 else screen_width + 25
                ...
                self.lasers_to_spawn.remove((spawn_time, y))
                
        # end the effect 
        if self.effect_active and now - self.effect_start_time >= self.effect_duration:
            self.effect_active = False
    ...
    def draw(self, surface):
        if self.warning_active:
            # draw warning img
            surface.blit(self.warning_image, self.warning_rect)
...
# fast moving ships that destroy most ships in burst attacks

class SpaceStrike(pygame.sprite.Sprite):
    def __init__(self, images_folder, screen_width, screen_height, speed=25):
        super().__init__()
        
        # load all aniamtion frames
        self.frames = [] # empty list for pictures
        for filename in sorted(os.listdir(images_folder)):
            if filename.lower().endswith(('.png', '.jpeg', '.jpg', '.ogg', '.bmp', '.gif')):
                file_img = pygame.image.load(os.path.join(images_folder, filename)).convert_alpha()
                self.frames.append(file_img)
        
        if not self.frames:
            raise ValueError("No images found in the folder of correct type")
            # return
        self.speed = speed
        ...
        self.positions = [
            pygame.Vector2(middle_x - spacing - 250, screen_height + 50),
            pygame.Vector2(middle_x - 100, screen_height + 50),
            pygame.Vector2(middle_x + spacing + 50, screen_height + 50)
        ]
        # animation controls
        self.frame_index = 0
        self.frame_timer = 0
        ...
        # rect for drawing our objects image on
        self.rects = [self.frames[0].get_rect(topleft=pos) for pos in self.positions]
        # state 
        self.alive = True
        self.start_time = pygame.time.get_ticks()
        self.shake_active = True
        self.shake_start_time = pygame.time.get_ticks()
        self.shake_duration = 1500 # 1.5 seconds
        self.shake_amplitude = 15
    ...
    def update(self):
        # move images upwards to the top from bottom
        for pos in self.positions:
            pos.y -= self.speed
            
        channel_sound_play = config.channel_14
            
        if not channel_sound_play.get_busy(): # check if it is not currently playing a sound
            channel_sound_play.set_volume(0.8)
            channel_sound_play.play(config.rapid_laser_fx, maxtime=1000)
        
        # update rects 
        for i, pos in enumerate(self.positions):
            self.rects[i].topleft = pos
            
        # damage all enemies 
        if not self.damage_done:
            for enemy in list(sprite_groups.enemy_group):
                if getattr(enemy, 'character_type', None) == 'enemy8':   
                    # enemy 8 gets damage directly to health bypass the shields
                    enemy.health -= 200
                else:# other enemies
                    apply_damage(enemy, 5000)
                    
            
            # break asteroids appart / to create damage zone 
            for asteroid in list(sprite_groups.asteroid_group):
                asteroid.break_apart(sprite_groups.asteroid_group, rocket_hit=False, space_strike=True)
            
            # damage has happend no need for more
        ...
        if all(pos.y + rect.height < 0 for pos, rect in zip(self.positions, self.rects)):
            self.alive = False 
            self.shake_active = False
            
    def get_shake_offset(self):
        if self.shake_active: 
            now = pygame.time.get_ticks() - self.shake_start_time
            if now < self.shake_duration:
                return (random.randint(-self.shake_amplitude, self.shake_amplitude),

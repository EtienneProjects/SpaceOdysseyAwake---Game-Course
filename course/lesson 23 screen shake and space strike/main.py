...
from projectiles import LaserLine, HorizontalManager, SpaceStrike
...
# have game in this new start game function
# ____ main ____
def start_game():
    global current_song
    
    
    # __ ensure global sprite groups exist and are also cleared when new game
    enemy_group.empty()
    ...
    enemy_lasers.empty()
    plasma_group.empty()
    space_strike_group.empty()
    
    
    # ___ create fresh player & other objects
    #create player object
    player = shipClass.Ship('player', 950, 750, 1, 10)
    
    # ensure player projectiles container is a group and linked again
    ...
    health_bar = shipClass.HealthBar(75, 970, player.health, player.health)
    shield_bar = shipClass.HealthBar(75, 990, player.shield, player.shield)
    
    #reset input flags so state does not get stale
    ...
    # long range strike 
    longRangeStrike = HorizontalManager(config.warning_img_path, config.laser_img_path, player, asteroid_group, enemy_group, interval_ms=10000)
    
    # temp surface for our screen shake surface to draw on and move for shake 
    ...
    while playing:
        ...
        # only start effect at level 10
        if wave_count >= 10:
            if config.long_strike_sound_played == False:
                config.channel_1.play(config.strike_detected)
                config.long_strike_sound_played = True # only play sound first time 
            longRangeStrike.update()
            longRangeStrike.draw(config.game_window)
            
            
            
            
        # space strike help 
        for ss in list(space_strike_group): # ss is space strike
            ss.update()
            ss.draw(config.game_window)
            
            # start shaking screen if not already active
            if not getattr(config, "shake_active", False):           
                config.shake_active = True
                config.shake_start_time = pygame.time.get_ticks()
                config.shake_duration = 1000 # 1 second
                config.shake_amplitude = 12
                    
            # delay 1 second before applying damage 
            if not getattr(ss, 'lower_health', False):
                if pygame.time.get_ticks() - ss.start_time >= 1000:
                    for enemy in enemy_group:
                        enemy.health = 0
                    for asteroid in asteroid_group:
                        asteroid.health = 0
                    ss.lower_health = True
                    
            if not ss.alive:
                space_strike_group.remove(ss)
                
                
        # screen shake    
        if getattr(config, "shake_active", False):
            now = pygame.time.get_ticks() - config.shake_start_time
            if now < config.shake_duration:
                shake_x = random.randint(-config.shake_amplitude, config.shake_amplitude)
                shake_y = random.randint(-config.shake_amplitude, config.shake_amplitude)
                
                # copy current window content to temp surface to blit
                temp_surface.blit(config.game_window, (0, 0))
                
                # clear window and blit with shake
                config.game_window.fill(config.BLACK)
                config.game_window.blit(temp_surface, (shake_x, shake_y))
                
            else:
                config.shake_active = False
        ...
        # space strike
        menu.drawText(f'Space Strike: {config.space_strike}', config.font, config.WHITE, 10, 1025)
        #score
        menu.drawText(f'Score: {config.score}', config.font, config.WHITE, 10, 1005)
        # wave count
        menu.drawText(f'Waves: {wave_count}', config.font, config.RED, 10, 1045)
        # wave count
        ...
        # Run our garbage collector here to clean old object where it can
        gc.enable()
        gc.collect(True)                      
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and config.space_strike >= 1 and config.shake_active == False:
                    ss = SpaceStrike("img/support", config.SCREEN_WIDTH, config.SCREEN_HEIGHT)
                    space_strike_group.add(ss)
                    ss.start_time = pygame.time.get_ticks() # timer for health delay
                    config.space_strike -= 1

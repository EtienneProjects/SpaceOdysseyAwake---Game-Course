...
for i in range(5):# flash 5 images in order
    menu_bg = pygame.image.load(os.path.join(f'img/menu/{i}.png')).convert_alpha()
...
def draw_button(text, x, y, width, height, inactive_col, active_col):
    ...
    txt_rect = txt_surface.get_rect(center=(x + width // 2, y + height // 2))
    config.game_window.blit(txt_surface, txt_rect)
...
def menu_screen():
    ...
    while in_menu:
        ...
        # draw score 
        drawText(f'Score:{config.score}', config.font_large, config.CAYAN, 870, 180)
